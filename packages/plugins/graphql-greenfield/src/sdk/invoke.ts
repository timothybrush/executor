...
// removed: v1 `resolveHeaders` (secret-backed header map via
// `resolveSecretBackedMap`). In v2 a connection resolves to a single credential
// value that the plugin renders through the integration's auth template at
// invoke time — there is no per-header secret map to resolve.
